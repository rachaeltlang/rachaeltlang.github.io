...
I use card for each article, which is then a list. I use a bar, specifically a tab bar at the bottom of the screen to navigate between screens.
...
I think this homework is more mobile-friendly than Badger Bakery because the previous and next buttons of Badger Bakery were more of a web design component. If it were more mobile-friendly, it could implement a gesture such as swiping left and right, or a carousel. This homework uses cards to show each article and is more similar to existing mobile apps.

3. What other gesture(s) could be added to Badger News? What additional functionality would these gesture(s) support?

One gesture that can be added to Badger News is pinch to zoom in on the text. The app would need to be responsive and increase the font size without allowing the text to overflow off the screen. Another gesture is flick, where users can swipe right on the screen to go back to the previous screen.

4. Is there opportunity to add any microinteraction(s)? If so, what would be the trigger-action pair(s)?

A microinteraction that could be added is a like button. The like button would be triggered when the user taps on it, and the action would be that the article is saved to a user's list of liked articles, stored in a third tab.