...
Followed
- low physical effort: the website is simple and straightforward and doesn't require significant effort to use
- tolerance for error: the website prevents misuse, for example, once a user is logged in, the login and register tabs are disabled so the user cannot attempt to login again. Similarly, once a user is logged out, they are only able login, not log out again.

Violated:
- size and space for approach and use: it can be argued that the card style of the posts is not the most effective use of space. Having posts appear in one column may be more effective and easy to read.
- simple and intuitive use: the website heavily depends on the English language. There are no images or icons that would allow a non-English speaker to navigate the website.
...
![sketch of improved webiste design with icons](_figures/step10.jpg)
